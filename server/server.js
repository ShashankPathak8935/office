...
//   const { userId } = req.params;
//   try {
//     await pool.query('UPDATE shashank_pathak.order_history SET status = $1 WHERE user_id = $2', ['user cancelled', userId]);
...
app.get('/order-history/:userId', async (req, res) => {
  ...
  try {
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*) AS total FROM shashank_pathak.order_history WHERE user_id = $1`,
      [userId]
    );
    ...
    const ordersResult = await pool.query(
      `SELECT oh.*, p.photo 
       FROM shashank_pathak.order_history oh
       JOIN shashank_pathak.products p ON oh.product_id = p.id
       WHERE oh.user_id = $1
...
app.get('/product-rating/:productId', async (req, res) => {
  ...
  try {
    // Fetch rating and rating count from shashank_pathak.products
    const queryText = `
      SELECT 
        rating, 
        rating_count 
...
//   try {
//     // Fetch rating and rating count from shashank_pathak.order_history based on user_id
...
app.get('/api/orders', async (req, res) => {
  ...
  let query = `SELECT * FROM shashank_pathak.order_history WHERE 1=1`;
...
//   try {
//     const result = await pool.query('SELECT DISTINCT user_id FROM shashank_pathak.order_history');
//     res.json(result.rows);
//   } catch (err) {
//     console.error(err);
//     res.status(500).send('Server Error');
...
app.get('/api/users', async (req, res) => {
  try {
    const result = await pool.query(`
      ...
      FROM shashank_pathak.users u
      JOIN shashank_pathak.order_history oh ON u.id = oh.user_id
    `);
    res.json(result.rows);
  } catch (err) {
    console.error(err);
